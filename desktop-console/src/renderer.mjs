...
const copy = {
  zh: {
    ...
    overviewTitle: "这台电脑的 Host", overviewDetail: "后台服务持有协作者、工具、工作与备份；关闭这个窗口不会让它离线。", systemHealthy: "服务可用", secureStorageLinux: "凭据由 Linux Secret Service 保护", secureStorageWindows: "凭据由 Windows 数据保护 (DPAPI) 保护", secureStorageUnknown: "凭据由系统安全存储保护", firewallBlocked: "防火墙还没放行局域网访问", firewallFix: "放行防火墙", firewallFixDetail: "手机需要通过局域网找到这台电脑。确认后会请求一次管理员授权，为 Host 添加一条仅限专用网络的入站规则；本机连接不受影响。如果当前 Wi-Fi 在 Windows 里被标记为「公用网络」，请先在系统设置里改成「专用网络」，否则规则不会生效。", firewallFixFailed: "防火墙规则没有建成，Windows 可能拒绝了授权或命令执行失败。请以管理员身份打开 PowerShell 运行：New-NetFirewallRule -DisplayName 'Aru Host (home)' -Direction Inbound -Action Allow -Protocol TCP -LocalPort {port} -Profile Private,Domain ，然后重新读取。", mobilePair: "连接手机", rename: "修改名称", pairedDevices: "已连接设备", capabilities: "能力状态", noDevices: "还没有其他设备", noDevicesDetail: "生成一次配对码，让手机确认这台电脑。",
    ...
    mobileIdentities: "手机协作者", mobileIdentityDetail: "手机协作者使用已连接的 Host 时会自动出现在这里。项目和页面保存在电脑，模型在手机端运行，无需给电脑配置模型。",
    ...
    previewUpdate: "更新 Windows 预览版 {version}", windowsPreview: "Windows 预览版 · 安装包尚未代码签名",
    updateAvailable: "可更新到 {version}", updateAction: "下载更新", updateHint: "会交给系统的软件安装器，Host 数据留在原目录。", operationDone: "已完成", destructiveTitle: "确认这个动作", failureTitle: "这里没有成功完成", stateUnreadable: "本地 Host 状态无法读取。已停止启动并保留原文件。请检查文件权限，或从已验证的备份恢复后重试；不要删除数据目录。", repair: "修复安全连接", chooseFolder: "选择文件夹", name: "名称", status: "状态", size: "大小", updated: "更新于", unknown: "未知",
    settings: "设置", toolAccess: "工具准入", allTools: "允许当前和未来全部工具", selectedTools: "只允许指定工具", toolNames: "工具名称，用逗号分隔", approveSession: "本次运行允许", archive: "归档", restore: "恢复", editSurface: "编辑页面", rollbackVersion: "回滚版本", newMemory: "添加记忆", newReference: "添加参考资料", recordTitle: "标题", recordContent: "内容", bundleEditUnavailable: "工程页面请回到它的电脑工作区编辑", consoleVersionNote: "由桌面 Console 发布", consoleRollbackNote: "由桌面 Console 回滚", uninstallHost: "移除这台电脑的 Host", uninstallHostDetail: "停止并移除当前用户的后台服务和设置，协作者、对话、页面与其他数据仍保留。", hostRemoved: "Host 已从当前用户移除，数据仍然保留。现在可以关闭窗口并卸载系统里的 Aru Host 应用。", enabled: "已启用", disabled: "未启用", thisConsole: "当前 Console",
  },
  en: {
    ...
    overviewTitle: "This computer's Host", overviewDetail: "The background service owns collaborators, tools, work, and backups. Closing this window does not take it offline.", systemHealthy: "Service available", secureStorageLinux: "Credentials protected by Linux Secret Service", secureStorageWindows: "Credentials protected by Windows Data Protection (DPAPI)", secureStorageUnknown: "Credentials protected by system secure storage", firewallBlocked: "The firewall has not opened LAN access yet", firewallFix: "Allow through firewall", firewallFixDetail: "Your phone reaches this computer over the local network. Confirming asks once for administrator approval and adds a private-network-only inbound rule for Host; local connections are unaffected. If Windows classifies the current Wi-Fi as a public network, change it to private first or the rule will not apply.", firewallFixFailed: "The firewall rule was not created; Windows may have declined the elevation or the command failed. Run this in an administrator PowerShell and refresh: New-NetFirewallRule -DisplayName 'Aru Host (home)' -Direction Inbound -Action Allow -Protocol TCP -LocalPort {port} -Profile Private,Domain", mobilePair: "Connect phone", rename: "Rename", pairedDevices: "Connected devices", capabilities: "Capabilities", noDevices: "No other devices yet", noDevicesDetail: "Issue a pairing code and confirm this computer from your phone.",
    ...
    mobileIdentities: "Phone collaborators", mobileIdentityDetail: "Phone collaborators appear automatically when using this connected Host. Projects and pages stay here; the phone runs the model. No computer model setup is needed.",
    ...
    previewUpdate: "Update Windows preview {version}", windowsPreview: "Windows preview · Installer is not code signed",
    updateAvailable: "Update to {version}", updateAction: "Download update", updateHint: "Your system installer handles the package and Host data remains in place.", operationDone: "Done", destructiveTitle: "Confirm this action", failureTitle: "This did not complete", stateUnreadable: "Local Host state could not be read. Startup stopped and the original file was preserved. Check permissions or restore a verified backup, then retry. Do not delete the data directory.", repair: "Repair secure connection", chooseFolder: "Choose folder", name: "Name", status: "Status", size: "Size", updated: "Updated", unknown: "Unknown",
    settings: "Settings", toolAccess: "Tool access", allTools: "Allow all current and future tools", selectedTools: "Allow selected tools only", toolNames: "Tool names, comma separated", approveSession: "Allow for this run", archive: "Archive", restore: "Restore", editSurface: "Edit page", rollbackVersion: "Rollback version", newMemory: "Add memory", newReference: "Add reference", recordTitle: "Title", recordContent: "Content", bundleEditUnavailable: "Edit project pages in their computer workspace", consoleVersionNote: "Published from Aru Host Console", consoleRollbackNote: "Rolled back from Aru Host Console", uninstallHost: "Remove this computer's Host", uninstallHostDetail: "Stop and remove this user's background service and settings while preserving collaborators, conversations, pages, and other data.", hostRemoved: "Host was removed from this user and its data was preserved. You can now close this window and uninstall the Aru Host system package.", enabled: "Enabled", disabled: "Disabled", thisConsole: "This Console",
  },
};
...
async function renderOverview() {
  ...
  const storageKey = state.bootstrap?.secretStorage === "windows-dpapi"
  ...
  content.querySelector("[data-fix-firewall]")?.addEventListener("click", async () => {
    if (!await confirmDialog(t("firewallFix"), t("firewallFixDetail"))) return;
    setBusy(true);
    try { await api.configureFirewall(); showToast(t("operationDone")); await boot(); }
    catch (error) {
      const failure = /firewall-rule-missing:(\d+)/.exec(error.message ?? "");
      showToast(failure ? t("firewallFixFailed", { port: failure[1] }) : error.message, true);
    }
    finally { setBusy(false); }
  });
...
function renderFailure(error) {
  const stateUnreadable = error.message.includes("host.state_unreadable");
  content.innerHTML = `<div class="failure-state"><div><div class="empty-symbol">!</div><h2>${esc(t("failureTitle"))}</h2><p>${esc(stateUnreadable ? t("stateUnreadable") : error.message)}</p><div class="head-actions">${stateUnreadable ? "" : `<button class="quiet-button" data-repair>${esc(t("repair"))}</button>`}<button class="primary-button" data-retry>${esc(t("retry"))}</button></div></div></div>`;
...
function renderUpdate() {
  ...
  button.textContent = t(state.update.preview ? "previewUpdate" : "updateAvailable", { version: state.update.version }); button.classList.remove("hidden");
}
