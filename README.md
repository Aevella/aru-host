...
当前版本是 **0.31.3**，Host 协议版本为 `stub-0.30`。它与当前 Aru TestFlight 版本配套使用，并提供面向普通 Mac 用户的 Apple 签名、公证安装包，面向 Debian/Ubuntu 桌面用户的 `x64` / `arm64` 安装包，以及 **Windows x64 预览版（未签名）**。

安装包、更新范围与升级说明见 [0.31.3 发布说明](docs/releases/0.31.3.md)。
...
```bash
sudo apt install ./aru-host-linux-0.31.3-x64.deb
```
